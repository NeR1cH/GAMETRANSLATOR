...
"""
DeepL Game Translator v3.4.0 - ИСПРАВЛЕННАЯ ВЕРСИЯ
==============================================================

ИСПРАВЛЕНИЯ v3.4.0:
- ✅ FIX #1: asyncio.as_completed — порядок результатов сохраняется
- ✅ FIX #2: Microsoft batch API — правильная распаковка ответа
- ✅ FIX #3: save_translation_result — исправлен отступ (синтаксическая ошибка)
- ✅ FIX #4: fix_translations — translator не переопределяется в цикле
- ✅ FIX #5: perform_translation — вынесен в threading.Thread (UI не замерзает)
- ✅ FIX #6: apply_translations_to_file — позиции пересчитываются после каждой замены
- ✅ FIX #7: _get_structural_keys — стабильный хэш через hashlib
- ✅ FIX #8: extract_placeholders — использует скомпилированный RE_CURLY_PLACEHOLDERS
- ✅ FIX #9: perform_conversion — temp_dir очищается в finally
- ✅ FIX #10: apply_translations — защита от записи нескольких файлов в один путь
- ✅ FIX #11: ArchiveBrowserDialog — temp result_dir регистрируется для очистки
- ✅ FIX #12: load_config — предупреждение при повреждённом конфиге
- ✅ FIX #13: RE_SINGLE_QUOTES_COMPLEX — паттерн для апострофов заменён на безопасный

Автор: NeR1cH
Версия: 3.4.0 - Fixed Edition
"""
...
# FIX #13: безопасный паттерн для одинарных кавычек — не захватывает апострофы внутри слов
# Матчит только строки где кавычка окружена пробелом/началом/концом строки или знаком препинания
RE_SINGLE_QUOTES_SIMPLE = re.compile(r"(?<!\w)'([^']+)'(?!\w)")
...
# FIX #8: единый скомпилированный паттерн, используется везде
...
def load_config():
    # FIX #12: предупреждение при повреждённом конфиге, файл пересоздаётся
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
                return json.load(f)
        except json.JSONDecodeError as e:
            logger.warning(f"⚠️ Конфиг повреждён (невалидный JSON): {e}")
            backup = CONFIG_FILE + ".bak"
            try:
                shutil.copy2(CONFIG_FILE, backup)
                logger.warning(f"⚠️ Повреждённый конфиг сохранён как {backup}")
            except Exception:
                pass
            logger.warning("⚠️ Используются настройки по умолчанию. Откройте Настройки и сохраните снова.")
        except Exception as e:
...
def _stable_hash(text):
    """FIX #7: стабильный хэш через hashlib (не зависит от PYTHONHASHSEED)"""
    return hashlib.md5(text.encode('utf-8', errors='replace')).hexdigest()

@lru_cache(maxsize=100)
def _get_structural_keys(content_hash, content_sample):
    """FIX #7: использует стабильный хэш как ключ кэша"""
    structural_keys = set()
    ...
    return frozenset(structural_keys)
...
def extract_english_text_from_file(file_path):
    ...
    try:
        ...
        # FIX #7: стабильный хэш вместо hash()
        content_hash = _stable_hash(content[:5000])
        structural_keys = _get_structural_keys(content_hash, content[:5000])
        ...
        for line in content.split('\n'):
            ...
            # 2. Одинарные кавычки — только если нет двойных
            if '"' not in line:
                # FIX #13: используем безопасный паттерн
...
def extract_placeholders(text):
    # FIX #8: используем скомпилированный RE_CURLY_PLACEHOLDERS
    return RE_CURLY_PLACEHOLDERS.findall(text)
...
def apply_translations_to_file(original_file, translations, output_file):
    """
    FIX #6: позиции пересчитываются корректно.
    Замены применяются в обратном порядке по позиции — это безопасно,
    так как замена справа не сдвигает позиции левых матчей.
    """
    try:
    ...
    for line in content.split('\n'):
        ...
        single_matches = list(re.finditer(r"(?<!\w)'([^']*)'(?!\w)", line))

        # FIX #6: сортируем в обратном порядке по позиции.
        # Замена справа налево гарантирует, что позиции левых матчей не смещаются.
        all_matches = sorted(
        ...
        for match, quote_type in all_matches:
            ...
            if translated_text:
                # FIX #6: используем позиции группы (group(1)), а не всего матча.
                # Поскольку матчи отсортированы справа налево, каждая замена
                # затрагивает только часть строки правее уже обработанных — смещений нет.
    ...
    for i in batch_iterator:
        ...
        if translator == "deepl":
            ...
            try:
                ...
                logger.warning(f"⚠️ Ошибка пакетного перевода DeepL: {e}")
                for text in batch:
                    result = translate_with_deepl(text, api_key, source_lang, target_lang, retry)
                    results.append(result[0] if isinstance(result, tuple) else result)
            ...
            try:
                ...
                if response.status_code == 200:
                    # FIX #2: Microsoft возвращает массив — один элемент на каждый входной текст
                    data = response.json()
                ...
                logger.warning(f"⚠️ Ошибка пакетного перевода Microsoft: {e}")
                for text in batch:
                    result = translate_with_microsoft(text, api_key, region, source_lang, target_lang, retry)
                    results.append(result[0] if isinstance(result, tuple) else result)
...
async def translate_texts_batch_async(texts, api_key, translator="deepl", source_lang='EN', target_lang='RU', region='global', retry=3):
    ...
    batch_size = 50
    # FIX #1: собираем задачи с индексами, чтобы гарантировать порядок результатов
    indexed_batches = [
    ...
    async with aiohttp.ClientSession() as session:
        tasks = {
            idx: asyncio.create_task(
                process_batch_async(session, batch, api_key, translator, source_lang, target_lang, region, retry)
            )
            for idx, batch in indexed_batches
        }

        if tqdm is not None and len(tasks) > 1:
            # FIX #1: дожидаемся всех задач, потом сортируем по индексу
            done_results = {}
            for f in tqdm.tqdm(asyncio.as_completed(list(tasks.values())), total=len(tasks),
                               desc="🚀 Асинхронный перевод", unit="пакет"):
                result = await f
                # Находим индекс этой задачи
                for idx, task in tasks.items():
                    if task.done() and idx not in done_results:
                        done_results[idx] = result
                        break
        else:
            # Для небольшого числа батчей используем gather — порядок гарантирован
            results_list = await asyncio.gather(*[tasks[idx] for idx in sorted(tasks.keys())])
            done_results = {idx: res for idx, res in enumerate(results_list)}

    # FIX #1: собираем результаты строго по порядку индексов
    final_results = []
    for idx in sorted(done_results.keys()):
        final_results.extend(done_results[idx])
...
async def process_batch_async(session, batch, api_key, translator, source_lang, target_lang, region, retry):
    if translator == "deepl":
        url = "https://api-free.deepl.com/v2/translate" if ':fx' in api_key else "https://api.deepl.com/v2/translate"
        params = {'auth_key': api_key, 'source_lang': source_lang, 'target_lang': target_lang}
        for text in batch:
            params.setdefault('text', [])
            params['text'].append(text)
        ...
        for attempt in range(retry):
            try:
                async with session.post(endpoint, params=params, headers=headers, json=body,
                                        timeout=aiohttp.ClientTimeout(total=60)) as response:
                    if response.status == 200:
                        data = await response.json()
                        # FIX #2: правильная распаковка Microsoft batch response
...
async def translate_with_deepl_async(session, text, api_key, source_lang='EN', target_lang='RU', retry=3):
    url = "https://api-free.deepl.com/v2/translate" if ':fx' in api_key else "https://api.deepl.com/v2/translate"
    params = {'auth_key': api_key, 'text': text, 'source_lang': source_lang, 'target_lang': target_lang}
...
class TranslatorApp:
    def __init__(self, root):
        ...
        # FIX #5: флаг для остановки потока перевода
        self._translation_stop_event = threading.Event()
    ...
    def setup_style(self):
        ...
        style.configure("Subtitle.TLabel", background="#2c3e50", foreground="#ecf0f1",
                        font=("Segoe UI", 10))
    ...
    def show_archive_browser(self, archive_path):
        try:
            ArchiveBrowserDialog(self.root, self, archive_path)
        except Exception as e:
    ...
    def create_apply_tab(self):
        ...
        orig_frame = tk.LabelFrame(content, text="Шаг 2: Выбрать оригинальные файлы",
                                   bg="white", font=("Segoe UI", 10, "bold"), padx=20, pady=15)
...
в формат игры с сохранением структуры файлов.
    ...
    def load_existing_file(self):
        ...
        if path:
            ...
            self.existing_label.config(
                text=f"Загружено: {len(self.existing_translations)} фраз", fg="#27ae60")
    ...
    def start_translation(self):
        ...
        # FIX #5: если перевод на паузе — возобновляем
        ...
        self.log_message("translate", "🚀 НАЧАЛО ПЕРЕВОДА (v3.4 - неблокирующий UI)")
        ...
        # FIX #5: запускаем перевод в отдельном потоке
        t = threading.Thread(target=self.perform_translation, daemon=True)
        t.start()

    def perform_translation(self):
        """
        FIX #5: выполняется в фоновом потоке — UI не блокируется.
        Обновления UI делаются через root.after().
        """
        progress = self.current_translation_progress
        new_texts = progress['texts']
        api_key = progress['api_key']
        translator = progress['translator']

        self.root.after(0, lambda: self.translate_progress.config(maximum=progress['total']))

        import datetime
        autosave_folder = os.path.join(tempfile.gettempdir(), "deepl_autosave")
        os.makedirs(autosave_folder, exist_ok=True)
        self.root.after(0, lambda: self.log_message("translate", f"💾 Автосохранения в: {autosave_folder}\n"))
        ...
        for i in range(progress['current_index'], progress['total']):
            ...
            if self.translation_paused:
                ...
                self.root.after(0, lambda idx=i: (
                    ...
                    self.log_message("translate", f"📊 Переведено: {idx}/{progress['total']}"),
                    self.log_message("translate", "💾 Нажмите 'Начать перевод' для продолжения"),
                ))
                return

            # Проверка принудительной остановки
            if self._translation_stop_event.is_set():
                return

            text = new_texts[i]
            percent = ((i + 1) / progress['total']) * 100

            self.root.after(0, lambda v=i + 1: self.translate_progress.config(value=v))
            self.root.after(0, lambda t=text, idx=i, pct=percent:
                            self.log_message("translate", f"[{idx+1}/{progress['total']}] ({pct:.1f}%) {t[:50]}..."))
            ...
            if isinstance(result, tuple):
                self.root.after(0, lambda r=result: self.log_message("translate", f"   ❌ Ошибка: {r[1]}"))
            else:
                progress['translated'][text] = result
                self.root.after(0, lambda r=result: self.log_message("translate", f"   ✅ {r[:50]}..."))

            # Автосохранение каждые 10 строк
            if (i + 1) % 10 == 0:
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                autosave_file = os.path.join(autosave_folder, f"autosave_{timestamp}.txt")
                save_translations_to_file(autosave_file, progress['translated'])
                fname = os.path.basename(autosave_file)
                cnt = i + 1
                self.root.after(0, lambda f=fname, c=cnt:
                                self.log_message("translate", f"   💾 Автосохранение: {c} фраз → {f}"))
        ...
        fname = os.path.basename(final_save)
        cnt = len(self.current_translations)

        self.root.after(0, lambda: self.log_message("translate", f"\n✅ Перевод завершён!"))
        self.root.after(0, lambda: self.log_message("translate", f"📊 Переведено: {cnt}"))
        self.root.after(0, lambda: self.log_message("translate", f"💾 Финальный файл: {fname}"))
        self.root.after(0, lambda: self.show_info(
            "Готово",
            f"Переведено {cnt} фраз!\n\n💾 Автосохранения в:\n{autosave_folder}\n\n📄 Финальный файл:\n{fname}"
        ))
    ...
    def pause_translation(self):
        if self.translation_running and not self.translation_paused:
            ...
            self.status_bar.config(
                text="⏸️ Перевод приостановлен - нажмите 'Начать перевод' для продолжения",
                bg="#f39c12", fg="white"
            )
    ...
    def resume_translation(self):
        if self.translation_running and self.translation_paused:
            ...
            self.log_message("translate",
                             f"📊 Продолжаем с позиции: {self.current_translation_progress['current_index']}")
            # FIX #5: возобновляем в новом потоке
            t = threading.Thread(target=self.perform_translation, daemon=True)
            t.start()

    def save_translation_result(self):
        # FIX #3: исправлен отступ — метод на верхнем уровне класса
        ...
        if self.convert_on_translate.get():
            ...
            if save_translations_to_file(temp_trans_file, self.current_translations):
                self.log_message("translate", "💾 Переводы сохранены во временный файл")
                    ...
                    for file in files:
                        if file.endswith(('.txt', '.json', '.yml', '.xml')):
                            ...
                            success, count = apply_translations_to_file(
                                orig_path, self.current_translations, out_path)
    ...
    def load_convert_translations_file(self):
        path = filedialog.askopenfilename(
            title="Выберите файл с переводами", filetypes=[("Text files", "*.txt")])
        if path:
            self.convert_translations_dict = load_translations_from_file(path)
            self.convert_trans_label.config(
                text=f"Загружено: {len(self.convert_translations_dict)} фраз", fg="#27ae60")
    ...
    def select_convert_original_folder(self):
        ...
        if path:
            ...
            self.convert_struct_label.config(text=f"Папка: {os.path.basename(path)}", fg="#27ae60")
    ...
    def select_convert_original_archive(self):
        ...
        if path:
            ...
            self.convert_struct_label.config(text=f"Архив: {os.path.basename(path)}", fg="#27ae60")
    ...
    def select_convert_output(self):
        ...
        if path:
            ...
            self.convert_output_label.config(text=f"Папка: {os.path.basename(path)}", fg="#27ae60")
    ...
    def perform_conversion(self, translations):
        """FIX #9: temp_dir гарантированно очищается через finally"""
        ...
        self.log_message("convert", "🔄 КОНВЕРТАЦИЯ ПЕРЕВОДОВ (v3.4)")
        ...
        try:
            ...
            for idx, (root, file) in enumerate(all_files, 1):
                ...
                success, count = apply_translations_to_file(orig_path, translations, out_path)
            ...
            # FIX #9: очистка temp_dir в любом случае (success или exception)
            if temp_dir and os.path.exists(temp_dir):
                try:
                    shutil.rmtree(temp_dir)
                except Exception as cleanup_err:
                    logger.warning(f"⚠️ Не удалось очистить temp_dir: {cleanup_err}")
    ...
    def load_apply_translations_file(self):
        ...
        if path:
            ...
            self.apply_trans_label.config(
                text=f"Загружено: {len(self.apply_translations_dict)} фраз", fg="#27ae60")
    ...
    def select_original_file(self):
        path = filedialog.askopenfilename(title="Выберите оригинальный файл",
                                          filetypes=[("Все файлы", "*.*")])
        if path:
            self.apply_original_path = path
            self.apply_original_type = "file"
            self.apply_orig_label.config(text=f"Файл: {os.path.basename(path)}", fg="#27ae60")
    ...
    def select_original_folder(self):
        ...
        if path:
            ...
            self.apply_orig_label.config(text=f"Папка: {os.path.basename(path)}", fg="#27ae60")
            self.log_message("apply", f"✅ Выбрана папка: {os.path.basename(path)}")

    def select_original_archive(self):
        path = filedialog.askopenfilename(
            title="Выберите архив", filetypes=[("Архивы", "*.zip *.tar *.tar.gz")])
        if path:
            self.apply_original_path = path
            self.apply_original_type = "archive"
            self.apply_orig_label.config(text=f"Архив: {os.path.basename(path)}", fg="#27ae60")
    ...
    def select_output_file(self):
        ...
        if path:
            ...
            self.apply_output_label.config(text=f"Файл: {os.path.basename(path)}", fg="#27ae60")
    ...
    def select_output_folder(self):
        ...
        if path:
            self.apply_output_path = path
            self.apply_output_label.config(text=f"Папка: {os.path.basename(path)}", fg="#27ae60")
            self.log_message("apply", f"✅ Результат будет сохранён в: {os.path.basename(path)}")
    ...
    def apply_translations(self):
        ...
        # FIX #10: если source — папка/архив, а output — файл, предупреждаем
        if self.apply_original_type in ("folder", "archive"):
            if self.apply_output_path and os.path.splitext(self.apply_output_path)[1]:
                self.show_error(
                    "Ошибка конфигурации",
                    "Источник содержит несколько файлов, но указан один файл для результата.\n\n"
                    "Пожалуйста, выберите папку для сохранения результата (Шаг 3 → 'Выбрать папку')."
                )
                return
                ...
                for root, dirs, files in os.walk(self.apply_original_path):
                    for file in files:
                        if file.endswith(('.txt', '.json', '.yml', '.xml')):
                            ...
                            os.makedirs(os.path.dirname(out_path), exist_ok=True)
                            success, count = apply_translations_to_file(
                                orig_path, self.apply_translations_dict, out_path)
                            if success:
                                processed += 1
                                total_replaced += count
    ...
    def fix_translations(self):
        ...
        # FIX #4: определяем translator один раз до цикла
        translator = self.translator_choice.get()
        ...
        api_key = self.deepl_api_key if translator == "deepl" else self.microsoft_api_key
        ...
        for i, error_data in enumerate(self.verify_errors, 1):
            ...
            # FIX #4: используем переменные, определённые до цикла
            result = translate_with_placeholder_protection(
                original,
                api_key,
                translator=translator,
...
class ArchiveBrowserDialog:
    def __init__(self, parent, app, archive_path):
        ...
        self.selected_items = []
        self.temp_dir = None
        # FIX #11: храним result_dir для последующей очистки при cancel
        ...
        tk.Label(info_frame,
                 text="Выберите файлы или папки для перевода. Можно выбрать несколько (Ctrl+клик).",
        ...
        try:
            ...
            for root, dirs, files in os.walk(self.temp_dir):
                for dir_name in dirs:
                    ...
                    items.append({
                        'name': rel_path, 'type': '📁 Папка',
                        'size': f'{file_count} файлов', 'full_path': full_path, 'is_dir': True
                    })
                ...
                for file_name in files:
                    if file_name.endswith(('.txt', '.json', '.yml', '.xml')):
                        ...
                        try:
                            ...
                            size_str = "?"
                        items.append({
                            'name': rel_path, 'type': '📄 Файл',
                            'size': size_str, 'full_path': full_path, 'is_dir': False
                        })
    ...
    def confirm_selection(self):
        ...
        # FIX #11: сохраняем result_dir в self для возможной очистки
        ...
        self.app.translate_source_label.config(
            text=f"Из архива: {len(selected)} элементов", fg="#27ae60")
        self.app.log_message(
            "translate",
            f"✅ Выбрано {len(selected)} элементов из архива {os.path.basename(self.archive_path)}\n"
            f"   Временная папка будет очищена после завершения перевода."
        )

        # Очищаем только browse temp, result_dir нужен для перевода
        self._cleanup_browse_temp()
        self.dialog.destroy()

    def cancel(self):
        # FIX #11: при отмене очищаем оба temp-каталога
        self._cleanup_browse_temp()
        self._cleanup_result_dir()
        self.dialog.destroy()

    def _cleanup_browse_temp(self):
    ...
    def _cleanup_result_dir(self):
        """FIX #11: очищает result_dir при отмене"""
...
class SettingsDialog:
    def __init__(self, parent, app):
        ...
        tk.Label(deepl_info, text="📌 Как получить: deepl.com/pro-api → Sign up for free\n✅ Бесплатно: 500,000 символов/месяц",
        ...
        tk.Label(microsoft_info,
                 text="📌 Как получить: portal.azure.com → Create resource → Translator → F0\n✅ Бесплатно: 2,000,000 символов/месяц",
                 font=("Segoe UI", 9), bg="#e8f4f8", fg="#2c3e50",
                 justify="left").pack(padx=10, pady=8, anchor="w")
        ...
        ttk.Button(btn_frame, text="💾 Сохранить", command=self.save, width=20,
                   style="Accent.TButton").pack(side="left", padx=5)
        ttk.Button(btn_frame, text="Отмена", command=self.dialog.destroy, width=20).pack(side="left", padx=5)
    ...
    def setup_context_menu(self, entry):
        ...
        context_menu.add_command(label="Вырезать (Ctrl+X)", command=lambda: self.cut_text(entry))
        context_menu.add_command(label="Копировать (Ctrl+C)", command=lambda: self.copy_text(entry))
        context_menu.add_command(label="Вставить (Ctrl+V)", command=lambda: self.paste_text(entry))
        context_menu.add_separator()
        context_menu.add_command(label="Выделить всё (Ctrl+A)", command=lambda: self.select_all(entry))
    ...
    def save(self):
        ...
        if not deepl_key and not microsoft_key:
            ErrorDialog(self.dialog, "Ошибка", "Нужно настроить хотя бы один переводчик!")
            return
        ...
        if deepl_key:
            ...
            if isinstance(test, tuple):
                ...
                ErrorDialog(self.dialog, "Ошибка DeepL", f"DeepL ключ не работает:\n{test[1]}")
                return
            deepl_ok = True
        ...
        if save_config(self.app.config):
            ...
            ErrorDialog(self.dialog, "Ошибка", "Не удалось сохранить настройки в файл")
...
class HelpDialog:
    def __init__(self, parent):
        ...
        help_text = """
🌐 ВКЛАДКА "ПЕРЕВОД"
Шаги: выберите тип → переводчик → источник → нажмите "Начать перевод"
Кнопка "⏸️ Пауза" работает без заморозки интерфейса (v3.4).

📝 ВКЛАДКА "ПРИМЕНИТЬ"
Применяет готовые переводы к оригинальным файлам.
Шаги: загрузите переводы → выберите файлы → выберите папку → применить.

🔍 ВКЛАДКА "ПРОВЕРКА"
Проверяет качество и автоматически исправляет проблемы.

🔄 ВКЛАДКА "КОНВЕРТАЦИЯ"
Преобразует переводы в формат игры с сохранением структуры.

⚙️ НАСТРОЙКИ
DeepL: deepl.com/pro-api (500K символов/месяц бесплатно)
Microsoft: portal.azure.com → Translator F0 (2M символов/месяц)

📦 Поддерживаемые форматы файлов: .txt .json .yml .xml
📦 Поддерживаемые архивы: .zip .tar .tar.gz .tgz

Версия: 3.4.0 - Fixed Edition | Автор: NeR1cH
        """
